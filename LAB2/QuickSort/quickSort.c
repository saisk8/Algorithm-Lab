...
void sortFiles(char * inPrefix,char * outPrefix){
	...
	for(i=100;i<=100000;i*=10){
		...
		if(fp!=NULL){
			...
			writeInReadMeFile("QuickSortFirstPivot",inputFileName);
			for(j=0;j<i;j++)
				fprintf(fp,"%d\n",arr[j]);
			fclose(fp);
			comparisions=0;
...
int main(){
	createOutputFilesDir("FixPivOut");
	char inputPrefix[3][20]={"InputFiles/in_asce","InputFiles/in_desc","InputFiles/in_rand"};
	char outputPrefix[3][20]={"FixPivOut/out_asce","FixPivOut/out_desc","FixPivOut/out_rand"};
	int i;
	createReadMeFile();
	for(i=0;i<3;i++){
