...
// Then calls Heapify() on it's parent to adjust heap -> totally takes O(log N)
// time
...
void dijkstra(struct node *adjacencyList[], int vertices, int startVertex,
  ...
  for (i = 1; i <= vertices; ++i) { // Untill there are vertices to be processed
    minVertex = extractMin(priorityQueue, vertices,
                           pos); // Greedily process the nearest vertex

    struct node *trav =
        adjacencyList[minVertex.vertex]; // Checking all the vertices adjacent
                                         // to 'min'
    ...
    while (trav != NULL) {
      ...
      if (distances[u] != INT_MAX && distances[v] > distances[u] + w) {
        // We have discovered a new shortest route, make the neccesary
        // adjustments in data
...
int main() {
  ...
  struct node *adjacencyList[vertices +
                             1]; // to use the array as 1-indexed for simplicity
  ...
  for (i = 1; i <= vertices; ++i) {
    printf("Vertex %d, Distance = %d, Parent = %d, ", i, distances[i],
           parent[i]);
    printf("Path = ");
